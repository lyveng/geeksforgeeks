...
public class InterpretationsOfArrayOfDigits {
  ...
  private static final String[] alphabet = {"", "a", "b", "c", "d", "e", "f", "g", "h", "i", "j",
      "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "v", "z"};
  ...
  public static Node createTree(int data, String curString, int[] arr) {
    ...
    if (arr.length > 0) {
      ...
      if (arr.length > 1) {
        node.right =
            createTree(arr[0] * 10 + arr[1], node.interpretation,
  ...
  public static void printAllInterpretations(int[] arr) {
    System.out
        .println(String.format("Printing interpretations for array %s", Arrays.toString(arr)));
    createTree(0, "", arr);
    System.out.println();
  }
